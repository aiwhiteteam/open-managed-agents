...
| Vaults and credentials | Partial | Basic lifecycle response shapes pass official SDK strict validation. API responses redact credentials, and archive/delete purges persisted secret values from the MVP generic table. Secure secret storage, OAuth enrollment/refresh, runtime resolution, permission enforcement, and webhook emission are TODO. |
