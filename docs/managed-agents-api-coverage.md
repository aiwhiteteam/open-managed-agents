...
Uploaded file mounts create session-scoped object-storage copies, validate absolute mount paths, and enforce the official 100 file resources per session limit. Production filesystem mount semantics are still tracked in `TODO.md`.
...
Deployment create/update validates the referenced agent, environment, and non-empty `initial_events`; short-form `agent="<agent_id>"` pins the latest active agent version. Deployment resources use the SDK session-resource union for files, GitHub repositories, and memory stores. Deployment responses omit write-only GitHub authorization tokens, and manual deployment runs mount deployment resources onto the created session.
