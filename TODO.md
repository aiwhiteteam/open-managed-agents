...
- Keep uploaded-file session mounts creating session-scoped object-storage copies. Staged upload completion is workspace-scoped. The official 100 file resources per session limit is enforced locally. Session-produced files should still become session-scoped file references.
...
- Implement deployment scheduler semantics: autonomous scheduled session creation, retries, and lease-safe workers. Cron/timezone validation, `upcoming_runs_at`/`last_run_at` bookkeeping, manual run session linkage, deployment `initial_events` delivery, deployment resource mounting, paused-manual-run behavior, terminal archive behavior, failed session-creation run records, and deployment-run vault validation have local coverage.
...
- Keep cron/timezone validation, schedule timestamp bookkeeping, deployment-run session linkage, deployment `initial_events` delivery, deployment resource mounting, paused-manual-run behavior, terminal archive behavior, failed session-creation run records, and vault validation covered.
