...
- Keep exact path and prefix lookups backed by indexed `managed_resources.name` as the stored `path_key`. SDK-compatible slash-prefixed path validation, memory content size, per-store count limits, memory-version `api_key_id`/`session_id`/`view` filters, deleted-version survival, live-head redact protection, and archived-store read-only behavior have local coverage.
