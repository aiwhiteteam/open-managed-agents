...
- Keep cron/timezone validation, required `user.message` deployment initial events, schedule timestamp bookkeeping, deployment-run session linkage, deployment `initial_events` delivery, deployment resource mounting, paused-manual-run behavior, terminal archive behavior, failed session-creation run records, and vault validation covered.
