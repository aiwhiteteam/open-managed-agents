...
- Preserve agent versioning semantics: agent updates require the current version, arrays replace wholesale, metadata merges/deletes intentionally with SDK-compatible bag limits, and delegated-agent rosters stay pinned rather than auto-updated.
...
- Implement outcome/grader loops with separate grader context, typed rubric inputs, max 20 iteration validation, and outcome evaluation events. A deterministic local `span.outcome_evaluation_end` MVP exists; real grader LLM loops remain TODO.
- Implement multiagent thread semantics: shared sandbox/filesystem/vault context, but separate persistent thread/context/event stream per agent. Delegated agent session threads are created from resolved/pinned rosters, including `self` entries resolved to concrete agent versions, and primary thread event listing includes unassigned session events; full delegated execution/routing remains TODO.
...
- Implement deployment scheduler production semantics: durable autonomous scheduling, retries, and lease-safe workers. An importable due-schedule tick creates scheduled deployment runs and linked sessions idempotently; cron/timezone validation, required `user.message` deployment initial events, `upcoming_runs_at`/`last_run_at` bookkeeping, manual run session linkage, deployment `initial_events` delivery, deployment resource mounting, paused-manual-run behavior, terminal archive behavior, primary-agent archive auto-archive/no-run behavior, failed session-creation run records, and deployment-run vault validation have local coverage.
...
- Keep the current passing SDK strict surface green: beta resource discovery, agent CRUD/versioned retrieve, agent versions, multiagent roster resolution, environment lifecycle/scope/work queue, session lifecycle/events/resources/threads, `user.tool_result`, `system.message`, files upload/list/download/delete, skill lifecycle/version lifecycle, vault/credential lifecycle including auth unions, memory store/memory/memory-version lifecycle and memory-version `api_key_id`/`session_id`/`operation`/`view` filters, deployment/deployment-run lifecycle including deployment `system.message` initial events, and user profile lifecycle.
- Expand pagination/filter contract tests from representative SDK coverage to exhaustive per-route edge cases, especially less common filters. Invalid cursor handling, expired cursor handling, SDK route-specific max limit clamping, timestamp aliases, invalid sort parameters, invalid enum filters, and core SDK pagination paths have test coverage.
...
- Keep Memory Store routes typed at the FastAPI boundary. Create/update routes use Memory Store-specific request models and typed response models are defined, while the MVP still stores records through the generic `managed_resources` table.
- Keep exact path and prefix lookups backed by indexed `managed_resources.name` as the stored `path_key`. SDK-compatible slash-prefixed path validation, required create content, `depth` rollups as `memory_prefix` items, memory content size, per-store count limits, no-op update behavior, stale precondition idempotency, delete `expected_content_sha256`, deleted-version response null content/hash/size, memory-version `api_key_id`/`session_id`/`operation`/`view` filters, deleted-version survival, live-head redact protection, and archived-store read-only behavior have local coverage.
...
- Keep cron/timezone validation, required `user.message` deployment initial events, deployment `system.message` ordering, deployment `resources`/`vault_ids` limits, list `agent_id`/`status` filters with `status` vs `include_archived` validation, deployment-run `has_error` semantics, schedule timestamp bookkeeping, deployment-run session linkage, deployment `initial_events` delivery, deployment resource mounting, paused-manual-run behavior, terminal archive behavior, primary-agent archive auto-archive/no-run behavior, failed session-creation run records, and vault validation covered.
- Implement production scheduler execution, retries, and lease-safe deployment-run workers. The core importable due-schedule tick is covered locally.
